...
Four lenses are implemented: ``peak5``, ``career``, ``per90`` and ``biggame``.
A lens answers one clearly-stated question rather than pretending to answer
"who was best", which is a choice about what matters that the project leaves
to the reader.

``teamfit``, a fifth lens ranking careers by output after regressing out club
strength, is deliberately not built. Measured, that residual correlated 0.996
with plain scoring output, because club strength explains under 1% of who
scores within a league-season; a lens that reduces to a lens already built
teaches nothing. See DECISION.md.
"""
